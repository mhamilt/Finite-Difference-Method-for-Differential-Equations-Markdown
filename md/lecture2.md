...
$$\phi = \frac{Kx^2}{2}.$$ Under such choice,
[\[eq:SHO\]](#eq:SHO){reference-type="eqref" reference="eq:SHO"} becomes
$$\label{eq:SHOreal}
    \frac{d^2 x}{dt^2} = - \omega_0^2 x,$$ where the radian natural
...
$$x(t) = a \cos(\omega_0 t) + b \sin{\omega_0 t}.$$ The constants $a,b$
are uniquely determined from the intial conditions. Setting $x(t=0)=x_0$
and $\frac{dx}{dt}(x=0)=v_0$, one gets $$\label{eq:SHOexact}
...
$$H(t) = \frac{m v_0^2}{2} + \frac{K x_0^2}{2} = H_0,$$ i.e.
...
difference scheme, consider $$\label{eq:Scheme1}
    \delta_{tt}x^n = -\omega_0^2 x^n.$$ Expanding out the operator, one
gets $$\label{eq:FDbasic}
    x^{n+1} = x^n(2-\omega_0^2 k^2) - x^{n-1},$$ and hence the update
...
$$\label{eq:ImplSchemeDef}
    g(x^{n+1})= f(x^n,x^{n-1}), \qquad \text{(implicit scheme)}$$ where
...
such that $$\label{eq:StabDef}
    |x^n| \leq C_\tau \sum_{m = 0}^M |x^m|$$ for a constant $C_\tau > 0$
...
reference="eq:ansatz"} is substituted: $$x^n = \hat x z^{n}.$$ In this
...
Hence, the solution is $$\label{eq:sol_z_SHO}
    x^n =  A_+ z^n_+ + A_- z^n_-,$$ for complex constants $A_\pm$. We
assume that the scheme is started using two starting values $x^0, x^1$
(obtained from $x_0$, $v_0$ of the continuous problem). Then
$$x^0 = A_+ + A_-, \quad x^1 = A_+ z_+ + A_- z_-.$$ From these, the
complex constants are obtained as $$\label{eq:ApAm}
...
$$\label{eq:StabCondSHO}
    k < \frac{2}{\omega_0},$$ which is an upper bound on the time step,
once the natural frequency of the oscillator is set. In the case of
oscillating solutions, one has $$z_\pm = r e^{\pm j \theta},$$ with
$r = 1$, and
...
reference="eq:StabDef"} is applied, to give $$\label{eq:SHObound} 
...
$$\label{eq:SHOen1}
    m \delta_{t\cdot}x^n (\delta_{tt}x^n +\omega_0^2 x^n) = 0$$ A couple
...
the total energy is $$\label{eq:ModEnSHO}
...
$$\label{eq:LTEdef}
    \delta_{tt}x(t_n) + \omega_0^2 x(t_n) = \varepsilon^n.$$ Using
...
if $$\lim_{k\rightarrow 0}\varepsilon^n = 0.$$ In practice, consistent
...
derivative: $$\label{eq:FourthOrderdtt}
...
Obviously, one can set $$x^0=x_0$$ For $x^1$, one possible solution is
...
with natural frequency $$\label{eq:ErrFreqs}
...
reference="eq:SHOreal"} can then be given as $$\label{eq:SHOLoss}
    \frac{d^2 x}{dt^2} = -\omega_0^2 x - 2c \frac{dx}{dt}.$$ Here,
...
[\[eq:ansatz\]](#eq:ansatz){reference-type="eqref"
reference="eq:ansatz"}. Thus, $$\label{eq:LossAnsatz}
...
the implicit definition of $\tau_{60}$ is obtained as $$\label{eq:tau60}
...
reference="eq:En1"}, we get $$\label{eq:EnBalLoss}
...
[\[eq:SHOLoss\]](#eq:SHOLoss){reference-type="eqref"
reference="eq:SHOLoss"} is obtained as $$\label{eq:FDSHOLoss}
    \delta_{tt}x^n = -\omega_0^2 x^n -2c \delta_{t\cdot}x^n,$$ yielding
an update equation
...
$$k < \frac{2}{\omega_0},$$ that is the same as
[\[eq:StabCondSHO\]](#eq:StabCondSHO){reference-type="eqref"
...
$$|z_\pm| = \sqrt{\frac{1-ck}{1+ck}}.$$ Thus, the numerical decay time
...
can be accomplised in the following way $$\label{eq:dttsq}
...
$$X(t) = e^{ct} x(t),$$ one gets
$$\frac{d^2X}{dt^2} + \left(\omega_0^2-c^2 \right) X = 0.$$ Thus, the
exact scheme
[\[eq:SHOexactNum\]](#eq:SHOexactNum){reference-type="eqref"
...
$\hat b = \hat f$, one gets $$\label{eq:steadyStSHO}
...
reference="eq:steadyStSHO"}: $$\label{eq:GreenSHO}
...
Consider now a harmonic input of the form $$\label{eq:harmoForceLinear}
    f(t) = F e^{j\omega t},$$ with $F \in \mathbb{C}$ being a complex
...
$$x(t) = X e^{j\omega t},$$ where we removed the index $st$ since we are
...
is obtained as $$\label{eq:XFlinearOsc}
...
scheme is obtained as $$\label{eq:shoFDForced}
...
$$(\delta_{t+}- \frac{k}{2}\delta_{tt})x^0 = v_0.$$ One has
